# Arkitektur
Dette dokumentet beskriver hvordan applikasjonen er bygget og hvordan et kall flyter gjennom lagene slik den fungerer nå.
...
flowchart LR
    ...
    UseCase --> EgrunnervervPort["EgrunnervervPort\nServiceNowAdapter"]
    EgrunnervervPort --> ServiceNowClient["ServiceNowClient\nRestClient + retry"]
    ServiceNowClient --> ServiceNowAPI["ServiceNow API"]

    subgraph Innkommende Web
      Controller
      OrgNoConv
      GlobalHandler[GlobalExceptionHandler]
    end

    subgraph Applikasjon
      UseCase
      Resolver
    end
    ...
    subgraph Utgående Infrastruktur
      Adapter
      VismaClient
...
sequenceDiagram
    ...
    participant E as ServiceNowAdapter
    participant SN as ServiceNowClient
    ...
    S->>E: syncSupplier(...)
    E->>SN: patch consumer (external ID)
    SN->>SNAPI: PATCH ... (ServiceNow)
    SNAPI-->>SN: OK
    SN-->>E: OK
    E-->>S: OK
    S-->>C: OK
...
- Leverandørflyt: Visma-klienten kaster egne unntak for hente/opprette/manglende selskapsmapping; adapter oversetter til [`SupplierSyncException`](src/main/kotlin/no/novari/flyt/egrunnerverv/okonomi/domain/error/SupplierSyncException.kt)-typer. ServiceNow-sync kan feile med `ServiceNowSyncException` når patch av ekstern ID ikke lykkes. Global handler returnerer passende HTTP/feilkoder.
...
## Drift/videre arbeid
- Tenant → adapter- og tenant → company-mapping må være komplett for alle tenants som skal bruke Visma; ServiceNow-konfig må også være satt, ellers feiler oppstart ([`AdapterConfigurationValidator`](src/main/kotlin/no/novari/flyt/egrunnerverv/okonomi/infrastructure/config/AdapterConfigurationValidator.kt)).
